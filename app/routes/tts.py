...
from app.services.generation_service import get_generation_for_user, validate_generation_ownership
...
async def generate_long_audio(
    ...
    background_tasks.add_task(background_long_audio, job_id, request, current_user.id)
    logger.info(f"Generation created for user_id={current_user.id} job_id={job_id}")
...
async def get_job_status(
    ...
    Enforces user ownership for the current authenticated user.
    """
    repo = AudioGenerationRepository(db)
    db_job = await get_generation_for_user(repo, job_id, current_user.id)

    mem_job = job_service.get_job(job_id)
    if mem_job:
        logger.debug(f"Returning in-memory job progress for job_id={job_id} user_id={current_user.id}")
        return mem_job
