...
export default function ImageHistory({
  ...
  emptyMessage = 'No images yet. Start generating to see them here!',
  onAuthRequired,
}: ImageHistoryProps) {
  const { jobs, fetchImages } = useImageJobsStore();
  ...
  if (isLoading) {
    return (
      <div className="w-full flex flex-col items-center justify-center py-12 gap-3">
        ...
        <p className="text-white/50">Loading your images...</p>
      </div>
    );
  }
    ...
    return (
      <div className="w-full bg-red-500/10 border border-red-500/30 rounded-2xl p-6 flex flex-col items-center gap-3">
        ...
        <div className="text-center">
          <p className="text-red-300 font-semibold">Failed to load images</p>
